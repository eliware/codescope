...
The remaining-file inventory samples each unsupplied file up to 100000 bytes. Oversized or exact-limit samples are omitted from content and reported as a bounded sample (`at least N sampled bytes`); the number is not presented as the file's total size.
