...
When `--usage` is enabled, the result includes `estimated_cost_usd` calculated from the selected model’s input, cached-input, cache-write, output, and long-context rates. `--dry-run` reports input-token cost only because no output is generated. Internal output writers return `undefined` or `{ written }` with an exact JavaScript character count; booleans, false, short counts, and unrelated values fail. Counts are JavaScript characters, not UTF-8 bytes. Successful provider response text is written unchanged; only fallback diagnostics are redacted, capped, and marked with `response_truncated` when capped.
