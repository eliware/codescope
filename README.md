...
Append `--usage` to either grouped (`codescope review all`) or direct (`codescope all`) review syntax to request usage metadata. `--dry-run` is supported on review and suggestion profile commands and returns that metadata as CodeScope output; it is not valid for `codescope prompt`, whose successful provider response text remains unchanged.
