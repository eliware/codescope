...
Running `codescope` with no command displays the single help page. Run a profile directly from the repository directory you want to review: `codescope implementation`, `codescope implementation-docs`, `codescope implementation-tests`, `codescope implementation-tests-docs`, `codescope refactor`, `codescope architecture`, `codescope new-features`, `codescope security`, `codescope performance`, `codescope reliability`, `codescope api-design`, `codescope dependencies`, `codescope observability`, `codescope accessibility`, `codescope release`, `codescope quick-wins`, `codescope prioritize`, `codescope tests`, `codescope tests-docs`, or `codescope docs`. File discovery is performed internally below the current working directory; test files mean only `*.test.mjs`, and symlinked test files or directories are excluded by the same symlink policy. The implementation-only suggestion profiles send complete implementation-file contents to the AI and return suggestions only; they do not modify files.
