...
test('validates the focused review and suggestion result contracts', () => {
  expect(isValidReviewResult({ issues: { security: [{ severity: 'P1', location: 'a:1', issue: 'x', ignore_example: '' }] }, verdict: 'block' }, prompt('issues', 'security'))).toBe(true);
  expect(isValidSuggestionResult({ suggestions: { security: [{ location: 'a:1', suggestion: 'x', rationale: 'y', ignore_example: '' }] } }, prompt('suggestions', 'security'))).toBe(true);
