...
test('uses native readers when adapters are not supplied', async () => {
  await expect(combineAllFiles(process.cwd(), { readDirectory: async () => [] })).resolves.toContain(
    '===== package.json =====',
  );
});
...
test('orders configs, documentation, code, tests, results, and other-file metadata', async () => {
  ...
  const result = await combineAllFiles('/repo', {
    readDirectory: async (directory) => directory.endsWith('repo')
      ? [
          ...names.filter((name) => !name.includes('/')).map((name) => ({ name, isFile: () => true })),
  ...
  await expect(combineAllFiles('/repo', { readDirectory: async () => [], readFileContents: async () => '{}', maxChars: 1 })).rejects.toThrow(/character limit/);
  await expect(combineSelectedFiles('/repo', { ...options, testResults: 'result' })).resolves.toContain('result');
});

test('reports package and configuration read failures clearly', async () => {
  await expect(combineAllFiles('/repo', { readDirectory: async () => [], readFileContents: async () => null })).rejects.toThrow(/non-string/);
  await expect(combineAllFiles('/repo', { readDirectory: async () => [], readFileContents: async () => { throw 'package failed'; } })).rejects.toThrow(/package failed/);
  await expect(combineAllFiles('/repo', { readDirectory: async () => [], readFileContents: async () => '{}', validateSymlinks: true, inspectFile: async () => ({ isSymbolicLink: () => true }) })).rejects.toThrow(/symlinked package/);
});
...
test('omits binary configs and truncates long text configs', async () => {
  ...
  const result = await combineAllFiles('/repo', {
    readDirectory: async (directory) => directory.endsWith('repo')
      ? [{ name: '.github', isDirectory: () => true }]
      : [{ name: 'large.yml', isFile: () => true }, { name: 'binary.yml', isFile: () => true }],
    readFileContents: async (file) => file.endsWith('package.json') ? '{}' : file.endsWith('binary.yml') ? Buffer.from([0, 1]) : longConfig,
  });
...
test('enforces selected-source limits', async () => {
  await expect(combineSelectedFiles('/repo', {
    maxChars: 1,
    readDirectory: async () => [{ name: 'app.mjs', isFile: () => true }, { name: 'guide.md', isFile: () => true }],
