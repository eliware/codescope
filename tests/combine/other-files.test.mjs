...
test('describes unsupplied text and binary files while excluding supplied content', async () => {
  ...
  expect(result).toEqual([
    'image.dat | binary | 3 bytes',
    'notes.txt | text | 2 lines | 3 bytes',
  ]);
});

test('returns no entries when all files are supplied elsewhere', async () => {
  await expect(describeOtherFiles('repo', ['package.json', 'README.md', '.github/ci.yml', 'src/app.mjs'])).resolves.toEqual([]);
});
