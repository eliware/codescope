...
test('reports findings, usage, cost, and elapsed time', () => {
  const result = reportBenchmarkResult('none', {
    output: JSON.stringify({ issues: { correctness: [{ issue: 'x' }] }, suggestions: { security: [{ suggestion: 'y' }] }, usage: { input_tokens: 1, output_tokens: 1 }, verdict: 'pass' }),
    elapsedMs: 120,
    code: 0,
  }, { elapsedMs: 20 }, 'gpt-5.6-luna');
  expect(result).toMatchObject({ issues: 1, suggestions: 1, elapsedMinusNpmTestMs: 100, verdict: 'pass' });
});
...
test('writes incremental and complete summaries', async () => {
  ...
  await writeBenchmarkSummary(file, {
    cwd: directory, npmTest, model: 'gpt-5.6-luna', pricing: {}, efforts: ['none', 'low'],
    results: [result], logs: directory,
  });
  ...
  await writeBenchmarkSummary(file, {
    cwd: directory, npmTest, model: 'gpt-5.6-luna', pricing: {}, efforts: ['none'],
    results: [result], logs: directory,
  });
...
test('reports malformed output without usage', () => {
  expect(reportBenchmarkResult('none', { output: 'bad', elapsedMs: 1, code: 1 }, { elapsedMs: 4 }, 'gpt-5.6-luna')).toMatchObject({
    issues: null, suggestions: null, estimatedCostUsd: null, verdict: null,
  });
});
