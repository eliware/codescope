...
test('classifies regular directories and files', () => {
  expect(classifyEntry({ isDirectory: () => true, isFile: () => false }, 'src')).toEqual({
    isDirectory: true,
    isFile: false,
    skip: false,
  });
  expect(classifyEntry({ isDirectory: () => false, isFile: () => true }, 'src')).toEqual({
    isDirectory: false,
    isFile: true,
    skip: false,
  });
...
test('rejects ambiguous, unknown, and throwing entries', () => {
  expect(() => classifyEntry({ isDirectory: () => true, isFile: () => true }, 'src')).toThrow(
