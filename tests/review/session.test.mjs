...
test('writes a fallback when provider output cannot be parsed', async () => {
  const output = [];
  await expect(executeReviewSession({
    client: { responses: { create: async () => ({ output: [{ type: 'function_call', name: 'submit_review', arguments: '{' }] }) } },
    request: reviewRequest,
    signal: new AbortController().signal,
    write: async (value) => output.push(value),
    dryRun: false,
...
test('handles custom structured JSON output and usage', async () => {
  const output = [];
  const result = await executeReviewSession({
    client: { responses: { create: async () => ({ output_text: '{"ok":true}', usage: { input_tokens: 2 } }) } },
...
test('handles combined tools and blocks on failed test evidence', async () => {
  const combinedRequest = {
    ...
    tools: [
      ...
      { name: 'submit_suggestions', parameters: { properties: { suggestions: { properties: { correctness: {} } } } } },
    ],
    tool_choice: 'auto',
  };
  const response = { output: [
    ...reviewResponse.output,
    { type: 'function_call', name: 'submit_suggestions', arguments: JSON.stringify({ suggestions: { correctness: [{ location: 'none', suggestion: 'No suggestions found.', rationale: '', ignore_example: '' }] } }) },
  ] };
  const result = await executeReviewSession({
...
test('returns a non-blocking result when failed evidence follows a block verdict', async () => {
  const response = { output: [{ type: 'function_call', name: 'submit_review', arguments: JSON.stringify({ issues: { correctness: [{ severity: 'P3', location: 'none', issue: 'No issues found.', ignore_example: '' }] }, verdict: 'block' }) }] };
  const result = await executeReviewSession({
    client: { responses: { create: async () => response } },
    request: reviewRequest,
    signal: new AbortController().signal,
...
test('preserves an untyped output failure without adding a provider code', async () => {
  await expect(executeReviewSession({
    ...
    request: reviewRequest,
    signal: new AbortController().signal,
    write: async () => { throw new Error('output failed'); },
    dryRun: false,
    usage: false,
    testResults: undefined,
